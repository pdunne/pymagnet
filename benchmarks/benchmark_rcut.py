...
def main():
    ...
    n_pts = X.size
    print(
        f"\nEvaluation grid: {N_PER_AXIS}³ = {n_pts} points  (±{XMAX:.0f} mm per axis)"
    )
    ...
    for r_cut in R_CUT_VALUES:
        ...
        print(
            f"  {label:>8}  {_fmt(t_med):>10}  {speedup:>7.2f}×"
    ...
    r_vals = np.array([r[0] for r, m in zip(results, finite_mask, strict=False) if m])
    t_vals = np.array(
        [r[1] * 1e3 for r, m in zip(results, finite_mask, strict=False) if m]
    )  # ms
    max_errs = np.array([r[2] for r, m in zip(results, finite_mask, strict=False) if m])
    rms_errs = np.array([r[3] for r, m in zip(results, finite_mask, strict=False) if m])
    nrmses = np.array([r[4] for r, m in zip(results, finite_mask, strict=False) if m])
    pw_rel_rmss = np.array(
        [r[5] for r, m in zip(results, finite_mask, strict=False) if m]
    )
    ...
    ax1b = ax1.twinx()
    ax1b.plot(
        r_vals, ref_ms / t_vals, "s--", color="darkorange", alpha=0.7, label="speedup"
    )
